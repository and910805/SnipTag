...
def main() -> None:
    ...
    try:
        ...
        print("流水號")
        check(namer.preview() == "週會_01.png", "空資料夾 → 週會_01.png")
        for expected in ("週會_01.png", "週會_02.png", "週會_03.png"):
        ...
        cfg["topic"] = "週會"
        check(namer.preview() == "週會_04.png", "切回舊主題接續 04")

        print("刪中間不會覆蓋")
        (tmp / "週會_02.png").unlink()
        check(namer.preview() == "週會_04.png", "空缺不回填，仍是 04")

        print("樣板")
        cfg["template"] = "{date}_{topic}_{n:03d}"
        today = dt.datetime.now().strftime("%Y%m%d")
        check(namer.preview() == f"{today}_週會_001.png", "日期樣板 + 三位數")
        cfg["template"] = "{topic}-{n}"
        check(namer.preview() == "週會-1.png", "無補零")

        print("格式與子資料夾")
        cfg["template"] = "{topic}_{n:02d}"
        cfg["format"] = "jpg"
        check(namer.preview() == "週會_01.jpg", "換副檔名後編號獨立計算")
        cfg["format"] = "png"
        cfg["subfolder_per_topic"] = True
        check(namer.target_dir() == tmp / "週會", "子資料夾路徑")
        check(namer.preview() == "週會_01.png", "子資料夾內從 01 開始")
