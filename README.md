...
| 📜 | **滾動截圖** | 長網頁、長文件：你自己捲，程式把畫面自動接成一張長圖 |
...
**網頁裡的元素也認得**：游標停頓一下，會透過 UI Automation 讀取無障礙樹，
把瀏覽器頁面內的文章區塊、圖片、表格等元素加進候選 ——
Chrome / Edge / Electron 這類「整個網頁是一塊畫布」的程式也能精準框選。

<img src="docs/window-detect.png" alt="輔助框自動偵測" width="820">

### 滾動截圖：整篇長文章一張搞定

系統匣選單 →「滾動截圖（長網頁）…」→ 框住要捲動的範圍，
然後**在那個視窗上用滾輪慢慢往下捲**，旁邊的小面板會即時顯示接了多長。
捲完按「完成」，整篇就以自動命名存成一張長圖。

- 捲動期間**擷取範圍會有藍色外框標示**，接不上時整圈轉橘色提醒
- 接合是**用畫面重疊比對**出來的，所以不管捲多捲少、忽快忽慢都接得準
- 捲太快接不上時會提醒你「往回捲一點再慢慢捲」，不會默默接錯
- 判定夠不夠可信是保守的：**寧可請你重捲，也不輸出一張錯位的長圖**
- 進度面板本身不會被拍進去（已從螢幕擷取中排除）

> 為什麼是自己捲而不是程式自動捲？合成滾輪事件在每個程式上的行為都不同
> （忽略、平滑動畫、慣性），你自己捲永遠是應用程式的原生行為，最可靠。
...
| `Ctrl+Shift+F3` | 關閉所有釘圖 |
| `Ctrl+F1` | 切換主題 |

系統匣選單的「更多截圖方式」裡另外有**延時 3／5 秒截圖**
（要先叫出下拉選單再拍時很好用）與**擷取作用中視窗**；
釘圖相關的操作則收在「釘圖」子選單。
...
├─ scroll.py      滾動截圖的拼接演算法（重疊比對）
├─ scrollsession.py 滾動截圖的進行中面板與範圍外框
├─ uielements.py  網頁內元素偵測（UI Automation）
...
- `test_scroll.py` —— 拼接演算法：造一張已知的長圖切成畫面餵回去，
  驗證結果**逐像素還原**；含假匹配、往回捲、重複畫面、行距陷阱等防呆
- `test_scrollsession.py` —— 滾動截圖的介面流程（警告、取消、上限自動收尾、
  範圍外框的穿透與變色）
- `test_uielements.py` —— UIA 元素階層的格式與合併邏輯
